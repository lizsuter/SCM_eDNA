...
So it retained ~68-99% of reads. Looking at stats output file, you can see the F and R primers were trimmed in almost all cases and the rev comp of each primer was trimmed in many cases too.
...
I am using blastn function in terminal, which can be installed with the blast package using conda `conda install blast`. I already had it installed but was having trouble bc v2.9 does not work on remote host (according to [this](https://www.biostars.org/p/400841/)). So I downgraded my blast to v2.6 with `conda install -c bioconda/label/cf201901 blast`


#### Test on subset of samples. I made a `test_sequences.fasta` which only has first 5 ASVs from `tax_sequences.fasta`.
```bash
blastn -query results/test_sequences.fasta \
-db nt \
-out results/test_blast_ASVs_perc_99.txt \
-remote
```
That generates full blast results with alignments of of all hits for every ASV and more.

#### Test2- only spit out best hit, collect table with PID, escore, etc
```bash
blastn -query results/test_sequences.fasta \
-db nt \
-out results/test2_blast.txt \
-remote \
-max_target_seqs 1 \
-outfmt 6 #\
```
OK that outfmt option #6 is a table that spits out:
...
But I also want to add info about blast subject. Try adding options:
...
#### Test3- take out `gapopen qstart qend sstart send` from table and add in `staxids sscinames sblastnames stitle`
```bash
blastn -query results/test_sequences.fasta \
-db nt \
-out results/test3_blast.txt \
-remote \
-max_target_seqs 1 \
-outfmt "6 qseqid sseqid pident length mismatch evalue bitscore staxids sscinames sblastnames stitle" 
```

It doesn't work to download the [taxdb](ftp://ftp.ncbi.nlm.nih.gov/blast/db/taxdb.tar.gz) because its working on remote server and database is local.

Instead, re-run without `sscinames sblastnames` then add taxonomy using R package.

#### Test4 Link Acc Numbers from with Taxonomy from NCBI. 
I found an R package for this called [Taxonomizr](https://github.com/sherrillmix/taxonomizr).


Download all tax assignments in NCBI and make SQLite database (this takes several hours). Store this file is a higer folder in a shared place instead of downloading it each time.

```{r}
prepareDatabase('databases/accessionTaxa.sql')
```

Read blast results from results table into R 
```{r}
blastResults<-read.table('results/test3_blast.txt',header=FALSE,stringsAsFactors=FALSE,fill=TRUE)
blastResults
```
...
```{r}
getTaxonomy(taxaId,'databases/accessionTaxa.sql')
```

Next append taxonomy to results file and export
