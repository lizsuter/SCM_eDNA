...
- [Exploratory Analyses](#exploratory-analyses)
  ...
  - [Ordinations on Trawl Data](#ordinations-on-trawl-data)
    - [Prepare the CPUE data](#prepare-the-cpue-data)
    - [CPUE PCA](#cpue-pca)
...
- Sara recommends at least a log-transformation to normalize for zeroes and high abundance/ schooling fish


Make a table of CPUE in the style of OTU tables (samples in rows/ species in columms), of the CPUE data. Trawl_counts has already been filtered so that it only includes those samples and species that are relevant to the eDNA study
```{r}
...
capture.output(log_transform_pca_envfit, file = "stats_results/pcoa_bray_envfit_CPUE.txt")

## Date (p = 0.04995), Bayside (p = 0.001998), and Station (p = 0.000999) are signficant
