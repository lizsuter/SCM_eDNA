...
- [Exploratory Analyses](#exploratory-analyses)
  ...
  - [Ordinations on Trawl Data](#ordinations-on-trawl-data)
    - [Prepare the Trawl Data](#prepare-the-trawl-data)
    - [CPUE PCA](#cpue-pca)
    - [Total Length (TL) PUE PCA](#tlpue-pca)
    - [TLxSF (shedding factor) PUE PCA](#tlxsf-pue-pca)
    - [CPUE PCoA Bray Curtis](#cpue-pcoa-bray-curtis)
    - [TL PUE PCoA Bray Curtis](#tlpue-pcoa-bray-curtis)
    - [TLxSF PUE PCoA Bray Curtis](#tlxsf-pue-pcoa-bray-curtis)
    - [CPUE Ordinations Summary](#cpue-ordinations-summary)
    - [Ordinations with Environmental Variables](#ordinations-with-environmental-variables)
      - [CPUE PCA with Environmental Variables](#cpue-pca-with-environmental-variables)
      - [CPUE PCoA with Environmental Variables](#cpue-pcoa-with-environmental-variables)
  - [Ordination Summary](#ordination-summary)
...
<iframe src="Embedded_figures/pca_lograt_eDNA.html" height="600px" width="100%" style="border:none;"></iframe>
...
<iframe src="Embedded_figures/pcoa_jaccard_eDNA.html" height="600px" width="100%" style="border:none;"></iframe>
...
<iframe src="Embedded_figures/pcoa_bray_eDNA.html" height="600px" width="100%" style="border:none;"></iframe>
...
<iframe src="Embedded_figures/pca_log_transform_CPUE.html" height="600px" width="100%" style="border:none;"></iframe>
...
```{r}
# convert to dataframe
TLSF.PUE_table_transform_cen_st <- data.frame(TLSF.PUE_table_transform_cen_st)
rownames(TLSF.PUE_table_transform_cen_st) <- c(TLSF.PUE_table_transform_cen_st$SampleID)
TLSF.PUE_table_transform_cen_st <- TLSF.PUE_table_transform_cen_st[,-1]
...
<iframe src="Embedded_figures/pcoa_bray_CPUE.html" height="600px" width="100%" style="border:none;"></iframe>
...
```{r}
# convert to dataframe
TLSF.PUE_table_transform <- data.frame(TLSF.PUE_table_transform)
rownames(TLSF.PUE_table_transform) <- c(TLSF.PUE_table$SampleID)
TLSF.PUE_table_transform <- TLSF.PUE_table_transform[,-1]
...
Plot in 2D. 
Actually didn't put any vectors in this plot because the only significant variable, station, is already color-coded and there are 11 stations so vectors would be messy.
```{r}
# Convert characters in metadata to factors
metadata_ordinations <- metadata_ordinations %>% mutate_if(sapply(metadata_ordinations, is.character), as.factor)
with(as.data.frame(metadata_ordinations), levels(Station))
...
# trim metadata to remove samples that are not in CPUE data table
metadata_ordinations <- metadata[metadata$SampleID %in% rownames(CPUE_table_transform),]
...
# fit environmental factors and save stats output
pcoa_bray_CPUE_envfit <- envfit(pcoa_bray, metadata_ordinations, permutations = 1000)
capture.output(pcoa_bray_CPUE_envfit, file = "stats_results/pcoa_bray_envfit_CPUE.txt")

## Bayside (p = 0.004995), and Station (p = 0.003996) are signficant. Date (p = 0.06593) is slightly above 0.05 threshold
