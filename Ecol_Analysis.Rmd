...
47 samples remaining with 368 ASVs
...
**Second** the Fourhorn sculpin (Myoxocephalus quadricornis) is actually an Arctic species. This ASV has 100% PID and 100% query cover to Myoxocephalus quadricornis & Myoxocephalus scorpius (another Arctic species) and 99.4% PID, 100% query cover to Myoxocephalus aenaeus. This latter one is actually the regional species, so change this ASV name to this one:
```{r}
...
**CHANGE TO USING FILTERED TRAWL DATA- DOESN'T INCLUDE NON-TARGET MIFISH-PRIMER SPECIES (CRABS AND ELASMOBRANCHS). also instead of abundance, it's CPUE**
...
pcoa_bray_envfit <- envfit(pcoa_bray, metadata_ordinations)
capture.output(pcoa_bray_envfit, file = "stats_results/pcoa_bray_envfit.txt")

# signficant variables include Datecode (p = 0.029), Month (p = 0.050), Station (p = 0.011), and, less significantly, Habitat (p = 0.086)

# fit the significant variables to the ordination
pcoa_bray_envfit_datecode <- envfit(pcoa_bray~Datecode+Habitat, data = metadata_ordinations, permutations = 9999)

# plot sig variables ontop of PCoA
plot(pcoa_bray)
plot(pcoa_bray_envfit_datecode, col = "black", cex=1.5)

## STOPPED HERE. KEEP EDITING ABOVE

# basic plot
ordispider(pcoa_bray_envfit)

plot(pcoa_bray)
plot(pcoa_bray_envfit)


# plot from plotly above. Modify to to 2D with same color scheme + vectors
plot <- plot_ly(pcoa_bray_df, type='scatter3d', mode='markers', 
                     x=~Dim2, y=~Dim3, z=~Dim1, colors=~brewer.pal(11,'Paired'),
        color=~Station, symbols = c('circle','diamond'), symbol=~Bayside)%>%  
  layout(font=list(size=12),
         title='PCoA Bray-Curtis Distance',
