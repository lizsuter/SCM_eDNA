...
Calculate the metric that Sara came up with: sum(total length) * shedding factor. This is a correction that takes into account the sums of length of each fish for each date/trawl and multiplies by a factor determined by how much they shed
```{r}
trawl_counts <- trawl_counts %>%
...
Make a table, in the style of OTU tables (samples in rows/ species in columms), of the CPUE data. Trawl_counts has already been filtered so that it only includes those samples and species that are relevant to the eDNA study
```{r}
...
## CONTINUE HERE- NEED TO MODIFY CLASS OF THE TABLE IN ORDER TO USE
## MAY 21
