...
rownames(CPUE_table_transform) <- c(CPUE_table_transform$SampleID)
CPUE_table_transform <- CPUE_table_transform[,-1]

log_transform_pca <- prcomp(CPUE_table_transform)
...
Total variance explained by first three axes= 28.6 + 19.5 + 13.7 = **61.8%**. 

## COME BACK HERE, CONSIDER WHETHER TO PLOT IN 2D OR 3D

Since the second and third axes are similar, plot in 3D with 3 axes.
...
## MAY 22- come back here to PCA above. The PerVar explained is good but the second axis is exagerated by one data point. Look into centering data, as described in several references from [above](#ordinations-on-cpue)

### PCoA Bray Curtis

Next, try a **Bray-Curtis** distance matrix with PCoA (continue to use log transformation). 
...
ggplot(bray_variances, aes(x = as.numeric(PCaxis), y = PercVar)) + 
  ...
  labs(x = "PC axis", y = "% Variance", title = "Bray-Curtis PCoA Screeplot")
```
The first two axes (12.9+11.0+9.9) are not as good as PCA. Total variance explained by first 3 PCs = **33.8%**)
...
## STOPPED HERE MAY 22. RE-ASSESS THE PCA AND PCOA ABOVE. MAY NEED CENTERING
