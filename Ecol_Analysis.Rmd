...
count_table_df <- as.data.frame(t(count_table))
colnames(count_table_df) <- count_table_df[1,]
count_table_df <- count_table_df[-c(1),]
rownames(count_table_df) <- NULL
...
rarecurve(as.numeric(count_table_df[,]), step=100, cex=0.5, ylab="ASVs", label=T)
##STOPPED HERE TUES NIGHT. ALSO CHECK ON RERUN OF BLAST STEP IN OTHER NOTEBOOK
```
